...
function generateDateData(config){
    ...
    var popickerYear = [];
    var valueField = config.valueField, displayField  = config.displayField, item, childField = config.childField;

    //start 默认追溯100年，end默认退后200年
    var startYear = (kit.isEmpty(config.start) || config.start == 'start') ? (yearNow-100) : config.start;
    var endYear = (kit.isEmpty(config.end) || config.end == 'end') ?  (yearNow + 200): config.end;
...
/*    for(var i = 1; i <= startYear; i++) {
    ...
    var normalData = generateAllMonths(config),
     leapData = normalData.concat(),
     leapMonth = createObj(valueField, displayField, toDate(2));//正常年份数据与闰年数据
    ...
    popickerDate.forEach(function(item, index){
      ...
      var year = item[valueField],
        bLeap = is_leap(year),
        data = bLeap ? leapData : normalData, specData;
  ...
  function generateDaysOfMonth(monthIndex, valueField, displayField, bLeap){
    var month = monthIndex +1, delta = bLeap ? 1 : 0,
    daysOfMonth, res = [], day = 1;
    switch(month){
      case 1:
      case 3:
      case 5:
